...
We can also include interaction terms in our model. There are two ways to do this. To include interaction between variables x and y, the term x:y should be included in form in the linearRegression call. linearRegression also supports input of terms separated by \*. For example, using form as y\~x1\*x2\*x3 is equivalent to using y\~x1 + x2 + x3 + x1:x2 + x1:x3 + x2:x3. To see this, let us consider the above model with all interaction terms involved: 

```{r}
fit_mlr_int<-linearRegression::linearRegression(mpg~cyl*disp*hp*wt, dat=mtcars)
