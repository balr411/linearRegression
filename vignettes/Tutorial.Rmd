...
```{r}
fit_mlr_int<-linearRegression::linearRegression(mpg~cyl+disp+hp+wt+cyl+cyl:disp+disp:wt+cyl:disp:hp, dat=mtcars)
...
```{r}
fit_mlr_int_lm<-lm(mpg~cyl+disp+hp+wt+cyl+cyl:disp+disp:wt+cyl:disp:hp, data=mtcars)
base::summary(fit_mlr_int_lm)
```

Again we can see the correctness of the function. Before moving on to benchmarking and system.time checks, it should be noted that the linearRegression function cannot take a factor as input, and no other formula formats are accecpted besides those written in this tutorial. If a factor is needed in your analysis, you must create your own dummy variables to include in the call to linearRegression. 
...
The above checks all parts of the implemented functions for correctness by checking against the output of the base/stats package lm, anova, and summary functions. "Not all equal" was not printed once, meaning our functions were correct on each run. 
